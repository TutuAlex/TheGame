...
int objloader::load(const char* filename)
{
	...
	for (int i = 0; i<coord.size(); i++) //and then go through all line and decide what kind of line it is
	{
		...
		else if ((*coord[i])[0] == 'v' && (*coord[i])[1] == 'n')    //if it's a normal vector
		{
			...
			normals.push_back(new vector3d(tmpx, tmpy, tmpz));      // do the same
			isnormals = true;
		}
		else if ((*coord[i])[0] == 'f')   //if it's a face
		{
			...
			{
				if (coord[i]->find("//") != std::string::npos)     //if it contains a normal vector, but not a texture coorinate
				{
					sscanf(coord[i]->c_str(), "f %d//%d %d//%d %d//%d %d//%d", &a, &b, &c, &b, &d, &b, &e, &b);      //read in this form file
																														...
																														//all of the materials
		{
			...
			{
				if (tmp[i][0] == '#')      //if its a comment do nothing
					continue;
				...
				else if (tmp[i][0] == 'N' && tmp[i][1] == 'i')      //the Iuknown property which i dont know what it is (still not used)
				{
		...
		{
			float u, v;
			sscanf(coord[i]->c_str(), "vt %f %f", &u, &v);     //read the uv vector3d
			texturevector3d.push_back(new texcoord(u, 1 - v));       //I push back 1-v instead of normal v, because obj file use the upper left corner as 0,0 coorinate
	...
	for (int i = 0; i<faces.size(); i++) //go throught all faces
	{
		...
		if (faces[i]->four)      //if quad
		{
			...
			if (istexture && materials[faces[i]->mat]->texture != -1)  //if there are textures
				glTexCoord2f(texturevector3d[faces[i]->texcoord[0] - 1]->u, texturevector3d[faces[i]->texcoord[0] - 1]->v);      //set the texture coorinate
...
unsigned int objloader::loadTexture(const char* filename)
{
	try {
		...
		//checks if there are no textures
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, img->w, img->h, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, img->pixels);
	...
	{
		std::cout << "img was not loaded. exception error ocurred with code: " + e  << std::endl;
...
void objloader::smoothnormals()
{
	for (int i = 1; i < vertex.size()+1; i++)
	{
		...
		{
			if (faces[j]->faces[0] == i || faces[j]->faces[2] == i || faces[j]->faces[3] == i)
			{
